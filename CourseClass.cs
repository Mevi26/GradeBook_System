...
namespace GradeBook_System
{
    class CourseClass
    {
        ...
        public bool insertCourse(string cName, int hr , string desc)
        {
            ...
            command.Parameters.Add("@desc", MySqlDbType.VarBinary).Value = desc;
        ...
        //create a function for course edit
        ...
        {
            MySqlCommand command = new MySqlCommand("DELETE FROM `course` WHERE `CourseId`", connect.getconnection);
            command.Parameters.Add("@id", MySqlDbType.Int32).Value = id;
